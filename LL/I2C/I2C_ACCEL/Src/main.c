...
#define LIS2DH12_I2C_ADDRESS 0x19 << 1
#define LIS2DH12_REG_CTRL_REG1 0x20
#define LIS2DH12_REG_OUT_X_L 0x28 << 1
/* USER CODE END PD */
...
void LIS2DH12_Write(uint8_t reg, uint8_t *data, uint8_t len);
...
int main(void)
{
  ...
  LL_GPIO_SetPinPull(ACCEL_PULL_UP_GPIO_Port, ACCEL_PULL_UP_Pin, LL_GPIO_PULL_UP);
	...
	LIS2DH12_Read(LIS2DH12_REG_OUT_X_L, 6);
  ...
  StartTransfers();
	...
	/*
	 Transmit message 
	Transmit_Data("AT+CMGS=\"+79534779719\"");
	Transmit_Data("Test message\x1A");
	Transmit_Data("AT+CMGR=1");	
	*/
...
void LIS2DH12_Read(uint8_t reg, uint8_t len)
{
    while (LL_I2C_IsActiveFlag_BUSY(I2C2) ||  LL_I2C_IsActiveFlag_TXIS(I2C2)) {}
...
void I2C2_3_IRQHandler(void)
{
		if (LL_I2C_IsActiveFlag_TXIS(I2C2))
    {
        ...
        LL_I2C_HandleTransfer(I2C2, LIS2DH12_I2C_ADDRESS, LL_I2C_ADDRSLAVE_7BIT, 1, LL_I2C_MODE_SOFTEND, LL_I2C_GENERATE_START_WRITE);
        LL_I2C_TransmitData8(I2C2, LIS2DH12_REG_OUT_X_L);
    }

    if (LL_I2C_IsActiveFlag_RXNE(I2C2))
    {
        LL_I2C_HandleTransfer(I2C2, LIS2DH12_I2C_ADDRESS, LL_I2C_ADDRSLAVE_7BIT, 6, LL_I2C_MODE_AUTOEND, LL_I2C_GENERATE_START_READ);

        for (uint8_t i = 0; i < 6; i++)
        {
            Rx_I2C2_Buffer[i] = LL_I2C_ReceiveData8(I2C2);
        }
    }
