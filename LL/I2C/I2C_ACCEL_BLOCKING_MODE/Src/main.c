...
#define RX_I2C2_BUFFER_SIZE 128
...
uint8_t Rx_I2C2_Buffer[RX_I2C2_BUFFER_SIZE] = {0,};

uint8_t Rx_I2C2_Index = 0;
...
void LIS2DH12_Read(uint8_t Reg, uint8_t Len);
/* USER CODE END PFP */
...
int main(void)
{
  ...
  LL_GPIO_SetPinPull(ACCEL_PULL_UP_GPIO_Port, ACCEL_PULL_UP_Pin, LL_GPIO_PULL_UP);
  ...
  StartTransfers();
	...
	*/
	...
	//LIS2DH12_Read(LIS2DH12_REG_CTRL_REG1, 1);
	//LL_mDelay(100);
	//LIS2DH12_Read(LIS2DH12_REG_OUT_X_L, 1);
  ...
  while (1)
  {
			LIS2DH12_Read(LIS2DH12_REG_OUT_X_L, 1);
			LL_mDelay(100);
			LIS2DH12_Read(LIS2DH12_REG_OUT_X_H, 1);
			LL_mDelay(100);
			LIS2DH12_Read(LIS2DH12_REG_OUT_Y_L, 1);
			LL_mDelay(100);
			LIS2DH12_Read(LIS2DH12_REG_OUT_Y_H, 1);
			
			
		/*
			Transmit_Data("AT+CNUM");
			Transmit_Data("AT+CPIN?");	
			Transmit_Data("AT+CCID");	
			Transmit_Data("AT+QGSN");	
			Transmit_Data("AT+CSCA?");	
			Transmit_Data("AT+CPBR=1,15");	
			Transmit_Data("AT+COPS?");	
			Transmit_Data("AT+CREG?");	
			Transmit_Data("AT+CSQ");	
			Transmit_Data("AT+CMGF=1");
			Transmit_Data("AT+CMGL=\"ALL\"");
		*/
...
{		
		...
		LL_I2C_HandleTransfer(I2C2, LIS2DH12_I2C_ADDRESS, LL_I2C_ADDRSLAVE_7BIT, 1, LL_I2C_MODE_SOFTEND, LL_I2C_GENERATE_START_WRITE);	
    ...
    LL_I2C_HandleTransfer(I2C2, LIS2DH12_I2C_ADDRESS, LL_I2C_ADDRSLAVE_7BIT, Len, LL_I2C_MODE_AUTOEND, LL_I2C_GENERATE_START_READ);
		...
		while (Len--)
		{
			...
			*Rx_ptr++ = LL_I2C_ReceiveData8(I2C2);
		}
...
void I2C2_3_IRQHandler(void)
{
    ...
    {
        LL_I2C_HandleTransfer(I2C2, LIS2DH12_I2C_ADDRESS, LL_I2C_ADDRSLAVE_7BIT, 1, LL_I2C_MODE_AUTOEND, LL_I2C_GENERATE_START_READ);
				
				if(Rx_I2C2_Index >= RX_I2C2_BUFFER_SIZE)
				{
					Rx_I2C2_Index = 0;
					memset(Rx_I2C2_Buffer, 0, RX_I2C2_BUFFER_SIZE);
				}
				Rx_I2C2_Buffer[Rx_I2C2_Index++] = LL_I2C_ReceiveData8(I2C2);
    }
...
void USART3_4_5_6_LPUART1_IRQHandler(void)
{
	...
	if(LL_USART_IsActiveFlag_IDLE(USART3))
	{
		...
		if(strstr((const char*)Rx_GSM_Buffer, "OK") == NULL && strstr((const char*)Rx_GSM_Buffer, "ERROR") == NULL)
		{
			return;
		}
		...
		GSM_Parse((const char*)Rx_GSM_Buffer, &GSM_Data);
